...
function Start(text, duration, linear)
    if type(text) ~= "string" then
        PrintError("le paramètre 'text' doit être de type :string")
        return
    end

    if tonumber(duration) == nil then
        PrintError("le paramètre 'duration' doit être de type :number")
        return
    end
    ...
    if options.Async == false then
        while Run do
            ...
            if IsControlJustPressed(0, cancelKey) and options.canCancel then
                OnComplete(true)
                TriggerEvent("TFS_progress:stop")
            end
...
RegisterNetEvent("TFS_progress:start")
RegisterNetEvent("TFS_progress:stop")
RegisterNetEvent("TFS_progress:custom")
RegisterNetEvent("TFS_progress:linear")
RegisterNetEvent("TFS_progress:minigame")

AddEventHandler("TFS_progress:start", Start)
AddEventHandler("TFS_progress:stop", Stop)
AddEventHandler("TFS_progress:custom", function(options)
    options.Async = false
    options.onStart = nil
    options.onComplete = nil
...
AddEventHandler("TFS_progress:linear", Linear)
AddEventHandler("TFS_progress:minigame", MiniGame)
