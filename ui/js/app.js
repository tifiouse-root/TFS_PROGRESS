const ui = document.getElementById('TFS_progress');
let running = false;
...
window.onData = function (data) {
    if ( data.MiniGame ) {
        ...
        if (data.display && !running) {
            if ( data.Type == 'linear' ) {
                customDial = new LineaTFS_progress({
                    w: data.Width,
                    h: data.Height,
                    x: data.x,
                    y: data.y,
...
function PostData(type, obj) {
    ...
    }).then(resp => resp.json()).then(resp => resp).catch(error => console.log('TFS_progress FETCH ERROR! ' + error.message));  
}